...
defineModule(sim, list(
  ...
  version = list(fireSense_dataPrep = "1.0.0"),
  ...
  parameters = rbind(
    ...
    defineParameter("parallel", "logical", FALSE, NA, NA,
                    desc = paste("Parallelise computations using `future`? Only applied to calculate initial fuel cover, using",
                                 "`multissession` by default and the number of workers defined by P(sim)$parallelCores.")),
    defineParameter("parallelCores", "integer", 1L, NA, NA,
                    desc = paste("Parallelise computations using `future`? Only applied to calculate initial fuel cover, using",
                                 "`multissession` by default and the number of workers defined by P(sim)$parallelCores.")),
...
prepFireSenseData <- function(sim) {
  ...
  fuelTypesCover <- Cache(calcFuelCoverWrapper,
                          ...
                          parallel = P(sim)$parallel,
                          workers = P(sim)$parallelCores,
                          cacheRepo = cachePath(sim),
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere("fireLocations", sim)) {
    ...
    SALarge <- st_transform(st_as_sf(sim$studyAreaLarge), crs = st_crs(fireLocations))

    ## "mask" a posteriori - having isues with SALarge when converted to SF , so will do in sp
    fireLocations <- st_crop(fireLocations, SALarge)

    ## project back to SA crs
    fireLocations <- st_transform(fireLocations, crs = crs(sim$studyAreaLarge, asText = TRUE))
