...
#    `functionName` is a function's name in your module (e.g., `fireSense_dataPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `E:/GitHub/LandscapesInMotion/R/SpaDES/m/fireSense_dataPrep/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_dataPrep")
  ...
  parameters <- list(
    ...
    fireSense_dataPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_dataPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_dataPrepEvent1(mySim)
  } else {
  ...
  if (exists("fireSense_dataPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_dataPrepEvent2(mySim)
  } else {
